...
const specialQuestions = [
    {
        ...
        text: '中国大陆 Top 3 大学是？',
        options: [
            {label: '上海交通大学', value: 1},
            {label: '复旦大学', value: 2},
            {label: '五角场文秘学院', value: 3},
            {label: '其他', value: 4}
        ]
    },
    {
        ...
        text: '上海交通大学的校训是？',
        options: [
            {label: '饮水思源，爱国荣校', value: 1},
            {label: '不要让这些数字因你而归零', value: 2},
            {label: '自由而无用', value: 3},
            {label: '“苟利国家生死以，岂因祸福避趋之”', value: 4},
        ]
    },
    {
        id: 'gary_gate_q1',
        ...
        options: [
            {label: '点踩 + 举报，我才是现在最想打人的！', value: 1},
            {label: '你拿满绩，我拿 3.3，我们都有光明的未来', value: 2},
            {label: '真的太可怜了......狠狠共情住了', value: 3},
        ]
    },
    {
        id: 'gary_gate_q2',
        ...
        options: [
            {label: '拍拍肩膀，深情地说：“我懂，那门课我拿了 3.86，其实跟挂了没区别。”', value: 1},
            {label: '建议他去树洞发帖，并承诺一定会号召大家给他点赞。', value: 2},
            {label: '一脸真诚地问他：“啊？原来真的会有人挂科吗？我还以为 jwc 默认最低分就是 3.8 呢......”', value: 3},
            {label: '刚才那题手滑了，我才不会跟 Gary 感同身受呢', value: 4}
        ]
    },
];
...
function getVisibleQuestions() {
    ...
    const garyGateIndex = visible.findIndex(q => q.id === 'gary_gate_q1');
    const sjtuerGateIndex = visible.findIndex(q => q.id === 'sjtuer_gate_q1');
    if (garyGateIndex !== -1) {
        visible.splice(garyGateIndex + 1, 0, specialQuestions[3]);
    }
    if (sjtuerGateIndex !== -1) {
        visible.splice(sjtuerGateIndex + 1, 0, specialQuestions[1]);
    }
    return visible;
}
...
function renderCurrentQuestion() {
    ...
    const current = visibleQuestions[app.currentIndex];
    ...
    card.innerHTML = `
        <div class="question-meta">
          ...
          <div>${getQuestionMetaLabel(current)}</div>
        </div>
        <div class="question-title">${current.text}</div>
        <div class="options">
    ...
    questionList.querySelectorAll('.option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                if (current.id === 'gary_gate_q1' && value !== 3) {
                    delete app.answers['gary_gate_q2'];
                }
                if (current.id === 'sjtuer_gate_q1' && value !== 1) {
                    delete app.answers['sjtuer_gate_q2'];
                }
...
function updateProgress(total) {
    const safeTotal = total || getVisibleQuestions().length;
    const currentStep = safeTotal ? Math.min(app.currentIndex + 1, safeTotal) : 0;
    const percent = safeTotal ? ((currentStep - 1) / safeTotal) * 100 : 0;
    progressBar.style.width = `${percent}%`;
    progressText.textContent = `${currentStep} / ${safeTotal}`;

    const remaining = safeTotal - currentStep;
    testHint.textContent = remaining > 13
        ? `点击选项继续（还剩 ${remaining} 道）`
        : `还剩 ${remaining} 道`;
    if (remaining === 24) {
        testHint.textContent = '其实现在退出也没有关系';
    } else if (remaining === 23) {
        testHint.textContent = '真的，不管是 FDTI 还是 SBTI 什么的，你不会真的信吧？';
    } else if (remaining === 22) {
        testHint.textContent = '告诉學生們，去做 FDTI，發帖子，點贊最高的，獎一个华为手表';
    } else if (remaining === 18) {
        testHint.textContent = 'MBTI 究竟是个什么样子，荣格搞了很多年，也并没有完全搞清楚。\n 可能B站UP主的思路比较好，搞了个 SBTI，可惜后来僵化了。';
    } else if (remaining === 15) {
        testHint.textContent = '该内容被作者删除';
    } else if (remaining === 0) {
        testHint.textContent = `完成后查看结果`;
    }
}
...
function computeResult() {
    ...
    if (app.answers['gary_gate_q2'] !== undefined && app.answers['gary_gate_q2'] !== 4) {
        finalType = TYPE_LIBRARY.GARY;
        secondaryType = bestNormal;
        modeKicker = '恭喜！你 roll 到了某 G 开头昵称的神入';
        badge = '匹配度 100% · *引起不适';
        sub = '4.0有错吗  大学里摆烂还正义了是吧';
        special = true;
    } else if (app.answers['sjtuer_gate_q2'] !== undefined && app.answers['sjtuer_gate_q2'] === 1) {
        finalType = TYPE_LIBRARY.SJTUER;
        secondaryType = bestNormal;
        modeKicker = '*已发现卧底*';
        badge = '卧底匹配度 100%';
...
function startTest(preview = false) {
    ...
    // 先打乱常规题目，再随机将门槛题插入（保留后续根据 q1 的答案插入 q2 的逻辑）
    const shuffledRegular = shuffle(questions);
    const queue = [...shuffledRegular];

    const sjtuerQ = specialQuestions.find(q => q.id === 'sjtuer_gate_q1');
    const garyQ = specialQuestions.find(q => q.id === 'gary_gate_q1');

    function insertRandom(arr, item) {
        const idx = Math.floor(Math.random() * (arr.length + 1)); // 0..length
        arr.splice(idx, 0, item);
    }

    // 始终插入 sjtuer 门槛题，gary 门槛题以 80% 概率插入（可按需调整）
    if (sjtuerQ) insertRandom(queue, sjtuerQ);
    if (garyQ && Math.random() < 0.8) insertRandom(queue, garyQ);
